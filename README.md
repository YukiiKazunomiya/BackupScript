...
**Note**: `SETUP.ps1` automatically configures the task to wake the computer from Sleep/Hibernate and allows it to run on battery power. 

**Solution**: 
- If your PC is completely **Shut Down** (Turned Off), the task cannot run. Ensure your PC is left in Sleep/Hibernate mode instead.
- If it's in Sleep mode but still doesn't run, check your Windows Power Options or BIOS/UEFI settings to ensure "Wake Timers" are enabled.
- Alternatively, you can check manually in Task Scheduler: open the task properties → **Conditions** tab → ensure "Wake the computer to run this task" is checked.
